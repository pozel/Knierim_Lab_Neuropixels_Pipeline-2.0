function detect_merge_peaks(fileName,fileDir,coordName,coordDir)
...
% hCfg.evtDetectRad = 0; %in um, distance to look for duplicate peaks
% hCfg.refracIntSamp = 0; %in samples, distance in time to look for duplicat peaks
...
maxTime = 60000; %in sec; takes the first maxTime seconds in the file
% maxTime = 1800; %in sec; takes the first maxTime seconds in the file
% maxTime = 300; %in sec; takes the first maxTime seconds in the file
...
switch dataType
    ...
    case 0
        ...
        uVPerBit = 0.6/512/1500*10^6;
        %uVPerBit = 2.34375;
        %this section was changed by VP - uVPerBit is incorrect? For a gain
        %of 50, the uVPerBit should be 0.6/512/50*10^6 = 23.4375
     
    case 1
        fs = 30000;
        nBit = 16384;    %amplitudes will be histogrammed over all bits
        %uVPerBit = 0.7629; check this value again as well, this doesn't
        %look right
        uVPerBit = 0.5/8192/80*10^6;
...
edges = (0:nBit);   %nBit+1 edges -> nBit bins. Overflow goes to top/bottom bin
    ...
    %VP preallocating arrays to a fixed size initially, and then will scale
    %as necessary once it exceeds the max
    numMaxEvents = 1000000000;
    allTimes = zeros(1, numMaxEvents, 'int32');
    allAmps = zeros(1, numMaxEvents, 'int32');
    allSites = zeros(1, numMaxEvents, 'int32');
    eventIndexStart = 1;
    eventIncrementSize = 10000000;
...
tic
    ...
    for i = 1:nBatch-1
         ...
         for jChan = 1:dataChan
             ...
             curEventsDetected = length(peaks);
             eventIndexEnd = eventIndexStart+curEventsDetected-1;

             %VP modified for preallocation version
%              if eventIndexEnd>numMaxEvents
%                  allAmps = [allAmps, zeros(1,eventIncrementSize)];
%                  allTimes = [allAmps, zeros(1,eventIncrementSize)];
%                  allSites = [allAmps, zeros(1,eventIncrementSize)];
%                  numMaxEvents = numMaxEvents+eventIncrementSize;
%              end
%                  
%              allAmps(eventIndexStart:eventIndexEnd) = amps;
%              allTimes(eventIndexStart:eventIndexEnd) = peaks+batchStart;

             allAmps = [allAmps,amps];
             allTimes = [allTimes,peaks+batchStart];            
             currSites = jChan*ones(1,numel(peaks));
             allSites = [allSites,currSites];
%              allSites(eventIndexStart:eventIndexEnd) = currSites;

             [tempHist, ~] = (histcounts(amps, edges));
             ampHist(jChan,:) = ampHist(jChan,:) + int32(tempHist);
             batchPeaks = batchPeaks + numel(peaks);
             
             eventIndexStart = eventIndexEnd+1;
...
%trimming arrays
% allAmps = allAmps(1:eventIndexStart-1);
% allTimes = allAmps(1:eventIndexStart-1);
% allSites = allAmps(1:eventIndexStart-1);
