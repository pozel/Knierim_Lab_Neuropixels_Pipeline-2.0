...
answer = inputdlg({'Rat', 'Date','Session'},'Enter the rat number and the date',[1 30],{'1050', '07262022','circtracksession'});
rat_no = answer{1};
date_str = answer{2};
ses_str = answer{3};
%% Selecting the files and directories
[binaryFile,path] = uigetfile(['C:\Analysis\' rat_no '\CatGT' '\catgt_' rat_no '_' date_str '_' ses_str '_g0\*.ap.bin'], ...
...
[csvFile,csvPath] = uigetfile(['C:\Analysis\', rat_no, '\Channels\*.csv'], ...
...
% Find stim artifact times
[stimtimesFile,path3] = uigetfile(['C:\Analysis\' rat_no '\CatGT' '\catgt_' rat_no '_' date_str '_' ses_str '_g0\ch3_stream.txt'], ...
                                'Select One or More Binary Files (After CatGT)','MultiSelect','on');
if isa(stimtimesFile,'double')
    return;
elseif isa(stimtimesFile,'char')
    stimtimesFile = {stimtimesFile};
end

% Vyash's code
Vyashpath = 'C:\Analysis\Winclust Pipeline\makeParms';
Vyashpath = uigetdir(Vyashpath,'Select Vyash''s Code Directory');
if isa(Vyashpath,'double')
    return;
end

selparentpath = uigetdir(['C:\Analysis\' rat_no '\Winclust\test'],'Select the main Directory for Saving CSC Files');
...
for l = 1:length(csvFile)
    ...
    if ~exist(selpath, 'dir')
        ...
        for k = 1:length(binaryFile)
            ...
            artifact_times = readtable([path3 'ch3_stream.txt']);
            artifact_timesamples = int64(table2array(artifact_times).*30000); %sec to sample times
        ...
        for j=range
            ...
            [time,data_np,header,ChannelNumber,SampleFreq,NumValidSamples] = read_bin_csc([selpath filesep 'CSC' num2str(j-1) '.ncs']);
            data_filtered = data_np; % filterlfp(time, data, [600 6000]);

            %remove artifact times here, in sec
            % #1 find artifact times from catgt stim times
            % #2 use these times to create a window
            % #3 run threshold & compare across channels to find individual
            % artifacts within stim time
            art_times = table2array(artifact_times);
            art_idx = [];
            for i = 1:length(art_times)
               art_idx(i,1) = find(art_times(i,1)<=time,1);
               data_filtered(art_idx(i,1)-25:art_idx(i,1)+75,1) = 0; %100, 25 before, 75 after samples removal (length of ministim)
            end

            mfb_thresh_artifacts = find(data_filtered<-0.0003 | data_filtered>0.0003); %samples that cross -0.0003
            for ii = 1:length(mfb_thresh_artifacts)
                data_filtered(mfb_thresh_artifacts(ii)-25:mfb_thresh_artifacts(ii)+75,1) = 0; %100, 25 before, 75 after samples removal (length of ministim)
            end
