...
% # channellistFile = input("Channel List File: ")
% # outputPath = input("Save Path: ") + '\\' + channellistFile

% # dataFilePath = glob.glob(basePath + '\\' + "*_binnoMFB.meta")[0] 
% # matFilePath = glob.glob(basePath + '\\' + "*_peaks.mat")[0]
% # channellistPath = r'D:\1085_NP2\aim2\channel_csvs_aim2' + '\\' + channellistFile + '.csv' #define channels here
...
for i = 1:length(all_data_files)
    for j = 1:length(all_channels_list)
        ...
        basePath = ["\\Mbi-jk-scanner\D\1085_NP2\aim3\" all_data_files(i) "\catgt_" all_data_files(i)]; % we loop through this in loop A
        basePath = join(basePath,"");
        channellistFile = all_channels_list(j); % we loop through this in loop B
        outputPath = ["C:\Analysis\1085\aim3\" all_save_files(i)]; % we loop through this in loop B
